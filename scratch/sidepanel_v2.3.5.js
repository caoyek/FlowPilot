...
function normalizePlusPaymentMethod(value = '') {
  ...
  const paypalHostedOnlyValue = typeof PLUS_PAYMENT_METHOD_PAYPAL_HOSTED_ONLY !== "undefined" ? PLUS_PAYMENT_METHOD_PAYPAL_HOSTED_ONLY : "paypal-hosted-only";
  const paypalHostedLinkOnlyValue = "paypal-hosted-link-only";
...
function updatePlusModeUI() {
  ...
  if (typeof rowPaypalHostedOnlyUrl !== 'undefined' && rowPaypalHostedOnlyUrl) {
    rowPaypalHostedOnlyUrl.style.display = enabled && selectedMethod === paypalHostedOnlyValue ? '' : 'none';
  }
  ...
  if (typeof plusPaymentMethodCaption !== 'undefined' && plusPaymentMethodCaption) {
    plusPaymentMethodCaption.textContent = method === gpcValue
      ...
      ? 'PayPal 无卡直绑链路'
      : method === paypalHostedOnlyValue
      ? 'PayPal 仅支付 (需提供长链)'
      : method === paypalHostedLinkOnlyValue
      ? 'PayPal 仅生成长链'
      : 'PayPal 订阅链路';
  }
...
function applySettingsState(state) {
  ...
  if (typeof inputPaypalHostedOnlyUrl !== "undefined" && inputPaypalHostedOnlyUrl) {
    inputPaypalHostedOnlyUrl.value = state?.paypalHostedOnlyUrl || latestState?.paypalHostedOnlyUrl || "";
  }
  if (typeof inputPaypalHostedOnlyUrl !== "undefined" && inputPaypalHostedOnlyUrl) {
    inputPaypalHostedOnlyUrl.addEventListener("change", async (e) => {
      await syncLatestState({ paypalHostedOnlyUrl: String(e.target.value || "").trim() });
    });
  }
